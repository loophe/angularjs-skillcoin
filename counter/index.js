...
// web3
import Web3 from './cgi-bin/web3.min';
...
*/
 ...
 if (typeof web3 !== 'undefined') {
    ...
    // call the name;
    counter.methods.name().call(function (err, result) {
        if (err) {
          return error(err);
        } else {
    ...
    // call the symbol;
      counter.methods.symbol().call(function (err, result) {
        if (err) {
          return error(err);
        } else {
          $("#instructor").html(result.toString()+"(标志 Symbol)");
          log("TonkenSymbol call executed successfully.");
          console.log(result);
        }
...
*/
    ...
    $('#increment').click(function (e) {
      ...
      counter.methods.cap().send(
        {from: '0x6c7F4f49e32137354F41F6cb50228917cE512958'},
        function (err, hash) {        
              if (err) {
                return error(err);
              }
